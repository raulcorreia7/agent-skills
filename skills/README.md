...
Skills marked `implicit` can activate from a matching request. In Codex, invoke
`manual` skills explicitly, for example `$audit`. Other clients may use a
different syntax. Compose skills only when their jobs are independently useful.
